...
Everything else configurable at runtime — CashApp cashtag/instructions, company/waybill info, pricing markup, the admin notification email address, and the scraper's own pbtmarketplace.com login — lives in the `settings` D1 table and is editable from the admin console's Settings page. There is no on-chain wallet, no BTC/USD price feed, and no user accounts anywhere in this project.
